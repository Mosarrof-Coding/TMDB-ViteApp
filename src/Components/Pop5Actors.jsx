...
function Pop5Actors({ actor, popActor, imgUrl }) {
  ...
  const customiseWinner = () => {
    const thisBtn = document.querySelector(`.winCards5-${id}`);
    if (thisBtn) {
  ...
  // Determine if the actor is a winner or nominee
  const makeWinner = (percent, win) => {
    if (percent === win) {
      return (
        <span
          className={`winCards5-${id} text-base lg:text-lg font-semibold px-4 py-[2px] my-3 rounded-full bg-[#01D277] inline-block text-white`}
        >
          Winner
        </span>
      );
