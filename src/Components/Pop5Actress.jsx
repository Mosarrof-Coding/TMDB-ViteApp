...
function Pop5Actress({ actress, popActresses, imgUrl }) {
  ...
  const customiseWinner = () => {
    const thisBtn = document.querySelector(`.winCards6-${id}`);
    if (thisBtn) {
  ...
  // Determine if the actress is a winner or nominee
  const makeWinner = (percent, win) => {
    if (percent === win) {
      return (
        <span
          className={`winCards6-${id} text-base lg:text-lg font-semibold px-4 py-[2px] my-3 rounded-full bg-[#01D277] inline-block text-white`}
        >
          Winner
        </span>
      );
